...
class GameMain:
  ...
  def create(self):
    ...
    mainPanel = Tk()
    canvas = Canvas(mainPanel, width=game.getWidth() * GameMain.SIZE, height=game.getHeight() * GameMain.SIZE, background = 'dark grey')
    canvas.pack(side=RIGHT, fill=BOTH, expand=YES)
    canvas2 = Canvas(mainPanel, width=game.getWidth() * GameMain.SIZE, height=game.getHeight() * GameMain.SIZE, background = 'white')

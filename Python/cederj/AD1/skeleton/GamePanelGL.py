...
class GameMain:
  ...
  def create(self,argv,npanels=0):
    ...
    game = GameImpl(GameMain.GRID_SIZE[0], GameMain.GRID_SIZE[1], BasicGenerator(GameMain.ICON_TYPES,GameMain.SEED))
  ...
  def main(self,argv=None):
      ...
      try: 
          try:
              ...
              opts, args = getopt.getopt(argv[1:], "ht:c:r:s:d:v:p:", ["help","types=","width=","height=","speed=","seed=","verbose=","panel="])
          ...
          for opt,arg in opts:  # something such as [('-h', '')] or [('--help', '')]
              if opt in ( "-h", "--help" ):
                 print ("Usage %s -t <icon_types> -c <grid_width> -r <grid_height> -s <speed> -d <seed> -v <level> -p <score_window>" % argv[0])
                 return 1
              elif opt in ("-c", "--width"):
